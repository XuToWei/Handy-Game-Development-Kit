...
public class DefaultAutoBindRuleHelper : IAutoBindRuleHelper
{
    ...
    private Dictionary<string, string> m_PrefixesDict = new Dictionary<string, string>()
    {
        ...
        {"SRect","ScrollRect"},
        {"Drop","Dropdown"},
        {"LoopVerSRect", "LoopVerticalScrollRect"},
        {"LoopHorSRect", "LoopHorizontalScrollRect"},
        {"LoopSHelper", "LoopScrollHelper"},
        {"DragHHelper", "DragHandlerHelper"},
        {"ImgList", "ImageList"},
        {"ParticleSystem", "ParticleSystem"},
        {"TMPTxt", "TextMeshProUGUI"}
    };

    public bool IsValidBind(string keyName, List<string> filedNames, List<string> componentTypeNames)
    {
        string[] strArray = keyName.Split('_');
        ...
        for (int i = 0; i < strArray.Length - 1; i++)
        {
            ...
            {
                Debug.LogError($"{keyName}的命名中{str}不存在对应的组件类型，绑定失败");
                return false;
            }
        }
